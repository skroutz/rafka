.PHONY: install build test teste2e testunit lint fmt clean run-rafka run-rafka-local testunit-local teste2e-local
...
build: fmt
	GOARCH=amd64 GOOS=linux CGO_ENABLED=1 go build -v -ldflags '-w -s -X main.VersionSuffix=$(shell git rev-parse HEAD)' -tags dynamic
